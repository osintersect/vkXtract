...
(() => {
  ...
  // src/entrypoints/offscreen-zip.ts
  function asMessage(value) {
    if (!value || typeof value !== "object") return {};
    return value;
  }
  function sanitizeZipFilename(input) {
    const clean = String(input || "").replace(/[<>:"/\\|?*\u0000-\u001F]+/g, "_").replace(/\s+/g, " ").trim();
    return clean || "vkxtract-case-package";
  }
  function resetZipState(root) {
    root.__vkxOffscreenZipExportId = "";
    root.__vkxOffscreenZipCancelled = false;
    root.__vkxOffscreenZipRunning = false;
    root.__vkxOffscreenZipAbortController = null;
  }
  function assertZipNotCancelled(root, exportId) {
    if (root.__vkxOffscreenZipCancelled) throw new Error("zip_export_cancelled");
    if (String(root.__vkxOffscreenZipExportId || "") !== String(exportId || "")) throw new Error("zip_export_cancelled");
  }
  async function relayZipProgress(exportId, text, pct) {
    try {
  ...
  async function relayZipDone(exportId, blobUrl, filename, text) {
    try {
      await chrome.runtime.sendMessage({
  ...
  async function relayZipError(exportId, error, text) {
    try {
      await chrome.runtime.sendMessage({
  ...
  function shouldRetryZipFetchStatus(status) {
  ...
  async function sleepWithAbort(ms, signal) {
    ...
    await new Promise((resolve, reject) => {
      ...
      const onAbort = () => {
        cleanup();
        reject(new Error("zip_export_cancelled"));
      };
      const cleanup = () => {
  ...
  async function fetchRemoteBlobWithRetry(remoteUrl, signal) {
    const delays = [0, 400, 1200];
    let lastErr = null;
    for (let attempt = 0; attempt < delays.length; attempt++) {
      if (signal?.aborted) throw new Error("zip_export_cancelled");
      if (delays[attempt] > 0) await sleepWithAbort(delays[attempt], signal);
      try {
        ...
        if (!res.ok) {
          if (shouldRetryZipFetchStatus(res.status) && attempt < delays.length - 1) {
            lastErr = new Error(`zip_export_failed: fetch ${res.status} for ${remoteUrl}`);
            continue;
          }
          throw new Error(`zip_export_failed: fetch ${res.status} for ${remoteUrl}`);
        }
        return await res.blob();
      } catch (err) {
        const cancelled = String(err?.message || "") === "zip_export_cancelled" || err?.name === "AbortError" || !!signal?.aborted;
        if (cancelled) throw new Error("zip_export_cancelled");
        lastErr = err;
        if (attempt >= delays.length - 1) break;
      }
    }
    throw lastErr || new Error(`zip_export_failed: fetch failed for ${remoteUrl}`);
  }
  async function blobForPlanEntry(entry, signal) {
    if (entry.sourceKind === "text") {
      return new Blob([String(entry.text || "")], { type: String(entry.mime || "text/plain") });
    }
    if (entry.sourceKind === "json") {
      return new Blob([JSON.stringify(entry.jsonValue ?? null, null, 2)], {
        type: String(entry.mime || "application/json")
      });
    }
    if (entry.sourceKind === "blob") {
      const blob = entry.blob instanceof Blob ? entry.blob : new Blob([]);
      if (entry.mime && blob.type !== entry.mime) {
        return new Blob([blob], { type: entry.mime });
      }
      return blob;
    }
    if (entry.sourceKind === "remote-url") {
      const remoteUrl = String(entry.remoteUrl || "").trim();
      if (!remoteUrl) throw new Error(`zip_export_failed: missing remote URL for ${entry.path}`);
      const fetched = await fetchRemoteBlobWithRetry(remoteUrl, signal);
      if (entry.mime && fetched.type !== entry.mime) {
        return new Blob([fetched], { type: entry.mime });
      }
      return fetched;
    }
  ...
  async function runZipExport(root, exportId, plan) {
    ...
    root.__vkxOffscreenZipCancelled = false;
    root.__vkxOffscreenZipRunning = true;
    try {
      await relayZipProgress(exportId, "Preparing ZIP archive…", 4);
      const writer = new ZipWriter(new BlobWriter("application/zip"), { zip64: true });
      for (let i = 0; i < entries.length; i++) {
        const entry = entries[i];
        const path = String(entry?.path || "").replace(/^\.\//, "").replace(/\\/g, "/").trim();
        if (!path) continue;
        assertZipNotCancelled(root, exportId);
        const pct = Number.isFinite(Number(entry.progressPct)) ? Math.max(4, Math.min(96, Number(entry.progressPct))) : Math.max(4, Math.min(96, 4 + Math.round((i + 0.25) / total * 90)));
        const text = String(entry.progressText || `Adding ${path}…`);
        await relayZipProgress(exportId, text, pct);
        let blob;
        try {
          blob = await blobForPlanEntry(entry, controller.signal);
        } catch (err) {
        ...
        try {
          ...
          const cancelled = String(err?.message || "") === "zip_export_cancelled" || err?.name === "AbortError" || !!controller.signal.aborted || !!root.__vkxOffscreenZipCancelled;
          if (cancelled) {
            throw new Error("zip_export_cancelled");
          }
          throw new Error(`zip_export_failed: ${path} :: ${String(err?.message || err || "zip_add_failed")}`);
        }
      }
      assertZipNotCancelled(root, exportId);
      ...
      const cancelled = String(err?.message || "") === "zip_export_cancelled" || !!root.__vkxOffscreenZipCancelled || !!controller.signal.aborted;
      if (cancelled) {
        await relayZipError(exportId, "zip_export_cancelled", "ZIP export cancelled.");
        return;
      }
      const msg = String(err?.message || err || "zip_export_failed");
      await relayZipError(exportId, msg, "ZIP export failed.");
    } finally {
      if (root.__vkxOffscreenZipAbortController) {
        try {
          root.__vkxOffscreenZipAbortController.abort();
        } catch {
        }
      }
      resetZipState(root);
  ...
  (() => {
    ...
    if (root.__vkxOffscreenZipBooted) return;
    root.__vkxOffscreenZipBooted = true;
    resetZipState(root);
    chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
      const req = asMessage(msg);
      if (req.target && req.target !== "offscreen-zip") return false;
      if (req.type === "VKX_OFFSCREEN_ZIP_PING") {
        const res = {
          ok: true,
          role: "offscreen-zip",
          ready: true,
          activeExportId: String(root.__vkxOffscreenZipExportId || "")
        };
        sendResponse(res);
        return false;
      }
      if (req.type === "VKX_OFFSCREEN_ZIP_START") {
        const exportId = String(req.exportId || "").trim();
        const plan = req.plan;
        if (!exportId) {
          sendResponse({ ok: false, error: "zip_export_missing_id" });
          return false;
        }
        if (root.__vkxOffscreenZipRunning) {
          sendResponse({
            ok: false,
        ...
        root.__vkxOffscreenZipCancelled = false;
        root.__vkxOffscreenZipRunning = true;
        const entryCount = Array.isArray(plan?.entries) ? plan.entries.length : 0;
        void runZipExport(root, exportId, plan || {
          pkgBase: "vkxtract-case-package",
          revealPath: "manifest.json",
          entries: []
        });
        const res = {
          ok: true,
      ...
      if (req.type === "VKX_OFFSCREEN_ZIP_CANCEL") {
        const exportId = String(req.exportId || root.__vkxOffscreenZipExportId || "").trim();
        if (!exportId) {
          sendResponse({ ok: false, error: "zip_export_not_active" });
          return false;
        }
        root.__vkxOffscreenZipCancelled = true;
        try {
          root.__vkxOffscreenZipAbortController?.abort();
        } catch {
        }
    ...
    try {
      console.log("[vkxtract] offscreen ZIP worker ready");
    } catch {
    }
  })();
})();
//# sourceMappingURL=offscreen-zip.js.map
